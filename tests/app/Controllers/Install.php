...
class Install extends BaseController
{
	...
	public function index()
	{
		...
		$validate->setRule('username', 'username', 'required|min_length[3]|max_length[20]|alpha_numeric');
		$validate->setRule('email', 'email', 'required|max_length[40]|valid_email');
		$validate->setRule('password', 'password', 'required|min_length[6]|max_length[200]');
		$validate->setRule('password_again', 'password confirmation', 'required|matches[password]');
		...
		if ($validate->withRequest($this->request)->run()) {
			...
			$fields = [
				...
				'last_activity' => ['type' => 'INT', 'constraint' => 10, 'unsigned' => TRUE, 'default' => '0'],
				'user_data'     => ['type' => 'TEXT'],
			];
			$this->dbforge->addField($fields);
			$this->dbforge->addKey('session_id', TRUE); // PRIMARY KEY
			$this->dbforge->addKey('last_activity');
			try{
			...
			$config_path = rtrim(APPPATH,'/').'/Config/App.php';
		...
		return view('pages/admin/install', [
			'data' => $data,
			'title' => 'Install'
		]);
